...
export const translations: Record<string, Translation> = {
  ...
  ru: {
    ...
    phoneAnalogy: "Представьте, что вы звоните кому-то, говорите {greeting}! а затем ставите их на удержание... 🤦‍♀️",
    dontDoThis: "Не делайте этого",
    insteadTryThis: "Вместо этого попробуйте это",
    noteText: (receiverName) =>
    ...
    ifBrusque: (greeting) =>
      `Если вы считаете, что просто сказать "${greeting}" и задать вопрос немного резко, вы все равно можете предварить свое сообщение любым количеством любезностей, которые считаете нужными.`,
    forExample: "Например:",
    exampleLong: (greeting) =>
      `${greeting}! Надеюсь, у тебя все хорошо. Мне нужна последняя презентация, когда у тебя будет минутка :)`,
    exampleMedium: (greeting) => `${greeting}, как дела? также, есть идея, когда это должно быть готово?`,
    exampleShort: (greeting) => `${greeting}, если ты не занят, не мог бы ты обновить эти NFR?`,
    asyncText: (greeting) =>
      `Это может показаться тривиальным, но задавая свой вопрос до получения этого первоначального приветственного ответа, вы также обеспечиваете асинхронную коммуникацию. Если другая сторона отсутствует, и вы уходите до ее возвращения, она все равно может ответить на ваш вопрос, вместо того чтобы просто смотреть на "${greeting}" и гадать, что она пропустила.`,
    everyoneHappy: "Когда все сделано правильно - все счастливы! 🎉",
    halfSerious: "Это только наполовину серьезно 😬 так что, пожалуйста, не",
    madLink: "злитесь",
    statusWarning: (greeting) =>
    ...
    enterGreeting: "Введите приветствие, которое нужно отговорить:",
    generate: "Создать",
